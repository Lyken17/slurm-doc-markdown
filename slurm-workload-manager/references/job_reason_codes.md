...
* **AssociationTimeLimit** — The job's association has reached its
  time limit.
* **BadConstraints** — The job's requirements cannot be satisfied.
  Used in cases when the job is never expected to be runnable, and typically
  results in the job being rejected or held.
...
* **Constraints** — The job's constraints cannot be satisfied at
  this time.
* **DeadLine** — This job cannot meet the configured Deadline.
...
* **JobHeldAdmin** — The job is held by a privileged user.
* **JobHeldUser** — The job is held by the user or an account
  coordinator. This is also applies whenever `scontrol uhold` is used.
* **JobHoldMaxRequeue** — Job has been requeued enough times to
  reach the MAX\_BATCH\_REQUEUE limit.
...
* **NonZeroExitCode** — The job terminated with a non-zero exit
  code.
* **None** — This state indicates that the job has not been

...
The primary display modes are *Jobs* and *Partitions*, each with a selection tab.
...
There is an *Admin Mode* option which permits the user root to modify many of
the fields displayed, such as node state or job time limit.
In the mode, a *Slurm Reconfigure* Action is also available.
It is recommended that *Admin Mode* be used only while modifications are
actively being made.
Disable *Admin Mode* immediately after the changes to avoid possibly making
unintended changes.

## ENVIRONMENT VARIABLES

Some **sview** options may be set via environment variables. These
...
The sview command can only be build if *gtk+-2.0* is installed.
...
[NAME](#lbAB): [SYNOPSIS](#lbAC): [DESCRIPTION](#lbAD): [ENVIRONMENT VARIABLES](#lbAE): [NOTES](#lbAF): [COPYING](#lbAG): [SEE ALSO](#lbAH)

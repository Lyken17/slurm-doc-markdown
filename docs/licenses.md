# Slurm Workload Manager - Licenses Guide
...
a specific host but are associated with the entire cluster.
...
the **AccountingStorageTRES** option of the slurm.conf.
...
Remote licenses **do not** offer any kind of integration with third party
...
(see [Dynamic Licenses](#dynamic_licenses)).

### Use Case

A site has two license servers, one serves 100 Nastran licenses provided by
...
licenses to cluster "pdf" and the remaining 30% to cluster "fluid".
...
absolute license counts rather than percentages.
...
need to be set aside temporarily.

E.g., on this cluster 100 "foobar" licenses are available, and we are
...
as it's likely that the job would fail to acquire the desired licenses.

If a further update (likely driven through cron) now reduces the

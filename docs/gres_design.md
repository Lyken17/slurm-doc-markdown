# Slurm Workload Manager - Generic Resource (GRES) Design Guide
...
Generic Resources (GRES) are resources associated with a specific node
...
named **gres\_list** that is intended for program use only. Each element
in the list **gres\_list** provides information about a specific GRES type
(e.g. one data structure for "gpu" and a second structure with information
about "nic"). The structures on **gres\_list** contain an ID number
...
**gres\_node\_state\_t**, **gres\_job\_state\_t**, and **gres\_step\_state\_t** in
...
node, job or step, then both **gres** and **gres\_list** will be NULL.
...
**node\_config\_load()** for each configured plugin. This can be used to
...
What does exist today is a pair of plugin APIs, **job\_set\_env()** and
**step\_set\_env()** which can be used to set environment variables for the

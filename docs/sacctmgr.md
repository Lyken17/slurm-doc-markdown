# Slurm Workload Manager - sacctmgr
...
establish 2 different limits: one for 16 CPUs and another for 32 GiB of memory.
...
Starting in Slurm 26.05, a TRES assignment can use a += or -= operator to add
...
**NOTE**: When dealing with Memory as a TRES the default units are MiB. A
...
: **Directory**= : Directory to store the archive data. : **Events** : When purging events also archive them. Default is to permanently delete purged records. : **Jobs** : When purging jobs also archive them, including scripts and environments. Default is to permanently delete purged records. : **JobScript** : When purging job scripts also archive them. Default is to follow **Jobs**. : **JobEnv** : When purging job environments also archive them. Default is to follow **Jobs**. : **PurgeEventAfter**= : Purge cluster event records older than the stated time. Units are months (default), days, or hours. For example, a value of "12hours" would purge all such records older than 12 hours. : **PurgeJobAfter**= : Purge individual job records older than the stated time (based on end time). Aggregated information will be preserved to "PurgeUsageAfter". Units are months (default), days, or hours. For example, a value of "12hours" would purge all such records older than 12 hours. **NOTE**: this includes job scripts and job environments if their specific purge options (**PurgeJobScriptAfter** and **PurgeJobEnvAfter**) are not set. : **PurgeJobScriptAfter**= : Purge job script records older than the stated time (based on end time). Units are months (default), days, or hours. For example, a value of "12hours" would purge all such records older than 12 hours. : **PurgeJobEnvAfter**= : Purge job environment records older than the stated time (based on end time). Units are months (default), days, or hours. For example, a value of "12hours" would purge all such records older than 12 hours. : **PurgeResvAfter**= : Purge reservation records older than the stated time (based on end time). Aggregated information will be preserved to "PurgeUsageAfter". Units are months (default), days, or hours. For example, a value of "12hours" would purge all such records older than 12 hours. : **PurgeStepAfter**= : Purge step records older than the stated time (based on end time). Units are months (default), days, or hours. For example, a value of "12hours" would purge all such records older than 12 hours. : **PurgeSuspendAfter**= : Purge job suspend records older than the stated time. Units are months (default), days, or hours. For example, a value of "12hours" would purge all such records older than 12 hours. : **PurgeTXNAfter**= : Purge transaction records older than the stated time. Units are months (default), days, or hours. For example, a value of "12hours" would purge all such records older than 12 hours. : **PurgeUsageAfter**= : Purge usage records (Cluster, Association, QOS and WCKey) older than the stated time (based on creation or last modified time). These tables are the source for reports generated by the [sreport](sreport.md)(1) command. Units are months (default), days, or hours. For example, a value of "12hours" would purge all such records older than 12 hours. : **Reservations** : When purging reservations also archive them. Default is to permanently delete purged records. : **Script**= : Run this script instead of the generic form of archive to flat files. : **Steps** : When purging steps also archive them. Default is to permanently delete purged records. : **Suspend** : When purging suspend records also archive them. Default is to permanently delete purged records. : **TXN** : When purging transaction records also archive them. Default is to permanently delete purged records. : **Usage** : When purging usage records also archive them. Default is to permanently delete purged records.

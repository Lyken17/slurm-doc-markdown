...
* **TRESBillingWeights**
  ...
  If `PriorityFlags=MAX_TRES` is set, the billable TRES is
  calculated as the sum of the following:
  + MAX of individual TRESs (those tied to a specific node, e.g., cpus, mem, gres)
  + SUM of all global TRESs (those available on any nodes, e.g., licenses)Using the same example above, the billable TRES will be:

  ```
  MAX(1*1.0, 8*0.25, 0*2.0) + (0*1.5) = 2.0
  ```

  If `PriorityFlags=MAX_TRES_GRES` is set, the billable TRES is
  calculated as the sum of the following:
  + SUM of all billable GRESs (e.g., GPUs)
  + MAX of other individual TRESs (those tied to a specific node, e.g., cpus, mem)
  + SUM of all global TRESs (those available on any nodes, e.g., licenses)With this flag set, the billable TRES will be:
